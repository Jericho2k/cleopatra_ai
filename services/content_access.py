...
async def resend_paid_content(fan_id: str, *, reference: str = "") -> dict[str, Any]:
    """Send a paid item again, free, and only once the platform accepted it.
    ...
    if reference:
        paid = [row for row in paid if str(row.get("reference") or "") == reference]
    if not paid:
    ...
    if not platform_message_id:
        # No receipt means no proof it arrived. Nothing is recorded as a repair
        # and the hold stays: an unverified repair must never be reported as
        # one.
        raise ContentAccessError(
    ...
    await save_message(
        ...
        media_context={
            ...
            "content_access_repair": {
                "restored_reference": str(row.get("reference") or ""),
                "media_ids": media_ids,
                "price_cents": 0,
    ...
    print(
        ...
        f"reference={row.get('reference')} media={media_ids} price_cents=0 "
...
async def resolve_content_access(
    ...
    *,
    ...
    if resolution == RESOLUTION_RESEND:
        result = await resend_paid_content(fan_id, reference=reference)
