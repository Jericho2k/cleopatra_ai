...
2. The semantic executor saves the accepted session plan before sending. A failed preflight left an unsent plan; a later blanket active-session check blocked it even after review was cleared. For persisted test fans, recovery now reuses only the exact accepted initial plan, with matching offer, set and price, no sent/purchased steps, and no pending payment. The delivery journal still blocks an outstanding delivery claim. This shortcut is not available to real fans, whose uncertain deliveries still require receipt recovery.
...
4. The semantic writer prompt explicitly preferred 1–2 short bubbles and illustrated two messages. The configured voice was already present in `evidence.creator_voice`; it was not missing. The prompt now explicitly refers to that existing evidence, uses contextual length and a single-message schema illustration, and discourages repeated paraphrases and stock reactions.
...
Regression coverage includes mirrored media through the semantic executor, local delivery receipt and pending-payment persistence; real-recipient rejection; exact unsent-plan recovery; bare-price validation; and valid responses over 25 words under the short persona setting. Model responses and external services are stubbed. These tests establish software behavior, not real-model conversational quality.
