...
class SimulateInboundRequest(BaseModel):
    ...
    fast: bool = True
    # One per Send press, generated by the browser. The server does not invent
    # it: the whole point is that the SAME submission, retried after a dropped
    # connection or a double click, is recognised as the same submission — and
    # only the client can assert that. Absent means "no idempotency", which is
    # what an older dashboard sends and is still one turn per POST.
...
async def simulate_inbound(
    ...
    """Accept one simulated fan message and start the REAL Full Auto turn.

    Returns as soon as the turn is RECORDED, not when it finishes:

        {"turn_id": "...", "status": "processing", ...}

    The pipeline then runs in the background against that durable row and the
    dashboard polls ``GET .../simulation/turn/{turn_id}``. This is the fix for
    the incident where a turn outlived the browser's 180-second ceiling: the UI
    reported a timeout, the backend carried on, the fallback answered, and the
    reply was persisted anyway — leaving the operator's screen and the database
    disagreeing about whether the turn had happened at all.

    One press of Send is one turn. ``idempotency_key`` makes a retried or
    double-submitted POST resolve to the turn that already exists, and a second
    CONCURRENT turn for the same fan is refused with 409 and the id of the one
    already running, so the browser can go and watch that instead.
    ...
    from services.simulation_turns import SimulationTurnBusy, start_turn
    ...
    # Owner diagnostics on the turn itself: the deadline it was started under
    # and, if it fails, the backend error. An agency gets the turn's status and
    # a correlating id — the same boundary the persisted "which brain wrote
    # this" marker respects on the messages the turn produces.
    ...
    try:
    ...
    except SimulationTurnBusy as busy:
        # 409, and the active turn with it. "Already running" is not an error
        # the operator has to resolve — it is the answer to "what should I be
        # watching", so the body carries it rather than only saying no.
        raise HTTPException(
            status_code=409,
            detail="A simulated turn is still running for this fan.",
            headers={"X-Simulation-Turn-Id": busy.active.id},
        ) from busy
    except HTTPException:
        raise
    except Exception as exc:
        # Recording a turn is an insert, so reaching here means the database
        # itself refused. Full traceback to the logs, the exception type and a
        # correlating id to the caller, no internals in the body — and, because
        # an unhandled exception's response never passed through CORS, a
        # handled 500 is also what stops the browser reducing this to "Failed
        # to fetch".
        error_id = uuid.uuid4().hex[:12]
        print(
        ...
        raise HTTPException(
            status_code=500,
            detail=(
                f"The simulated turn could not be started "
    ...
    return {
        ...
        "simulation": True,
        "created": created,
        **turn.public_view(diagnostics=diagnostics),
    }


async def _simulation_turn_response(
    request: Request,
    creator_id: str,
    fan_id: str,
    turn,
) -> dict:
    """One turn plus the creator messages it has produced so far.

    The transcript rows travel with the turn so a completed poll renders the
    reply without a second round trip and without the operator pressing
    Refresh. They pass through the same redaction the synchronous response used
    to: an agency is told which AI stack answered, never what it routes to.
    """
    ...
    diagnostics = request_is_platform_operator(request)
    view = turn.public_view(diagnostics=diagnostics)

    rows: list[dict] = []
    if turn.creator_message_ids:
        wanted = set(turn.creator_message_ids)
        rows = [
            row
        ...
        if not diagnostics:
            rows = public_message_rows(rows)

    return {
        "status": "ok",
        "simulation": True,
        **view,
        "creator_messages": rows,
    }


@app.get("/creator/{creator_id}/fan/{fan_id}/simulation/turn/{turn_id}")
async def read_simulation_turn(
    creator_id: str,
    fan_id: str,
    turn_id: str,
    request: Request,
) -> dict:
    """Poll one simulated turn. A pure read — it never generates anything.

    The pipeline runs exactly once, from the task the POST spawned. However
    often this is called, and however many browsers call it, no model is
    invoked here.
    """
    from services.simulation_turns import get_turn

    await _require_simulatable_fan(request, creator_id, fan_id)
    turn = await get_turn(creator_id, fan_id, turn_id)
    if turn is None:
        # The same 404 the rest of the simulator uses: a turn id belonging to
        # another tenant must be indistinguishable from one that never existed.
        raise HTTPException(status_code=404, detail="Resource not found")
    return await _simulation_turn_response(request, creator_id, fan_id, turn)


@app.get("/creator/{creator_id}/fan/{fan_id}/simulation/turn")
async def read_latest_simulation_turn(
    creator_id: str,
    fan_id: str,
    request: Request,
) -> dict:
    """The newest turn for this conversation, if there is one.
    ...
    or to render the one that finished while it was away. Also a pure read.
    """
    from services.simulation_turns import latest_turn

    await _require_simulatable_fan(request, creator_id, fan_id)
    turn = await latest_turn(creator_id, fan_id)
    if turn is None:
        return {"status": "ok", "simulation": True, "turn_id": None}
    return await _simulation_turn_response(request, creator_id, fan_id, turn)
